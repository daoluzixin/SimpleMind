"""
MiniMind 全量指令微调脚本（Full SFT）

本脚本实现了 MiniMind 模型的全量指令微调（Supervised Fine-Tuning），
即在预训练模型基础上，使用标注的对话数据进行有监督学习。

与预训练的区别:
- 预训练: 所有 token 都参与损失计算（自监督）
- SFT: 只有 assistant 回复部分的 token 参与损失计算（通过 loss mask 实现）

训练流程与 train_pretrain.py 基本相同，主要差异:
1. 使用 SFTDataset 数据集（支持对话格式和 loss mask）
2. 默认学习率更小（1e-5 vs 5e-4），避免破坏预训练知识
3. 默认基于 pretrain 权重训练
4. 默认 max_seq_len 更长（768 vs 340），适应对话场景
"""
import os
import sys
...
def train_epoch(epoch, loader, iters, start_step=0, wandb=None):
    ...
    训练流程与预训练相同，但 SFTDataset 返回的 labels 已经过 loss mask 处理:
    - 用户输入部分的 labels 设为 -100（不参与损失计算）
    - 只有 assistant 回复部分的 labels 保留真实 token ID
    ...
    for step, (input_ids, labels) in enumerate(loader, start=start_step + 1):
        ...
        with autocast_ctx:
            ...
            loss = res.loss + res.aux_loss  # 总损失 = SFT 交叉熵损失 + MoE 辅助损失
...
if __name__ == "__main__":
    ...
    parser.add_argument("--learning_rate", type=float, default=1e-5, help="初始学习率（SFT 学习率远小于预训练）")
    ...
    parser.add_argument('--from_weight', default='pretrain', type=str, help="基于哪个权重训练，默认基于预训练权重")
