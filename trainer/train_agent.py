"""MiniMind Agent RL（强化学习工具调用训练）脚本

本脚本实现了 Agent 的强化学习训练，使模型学会:
1. 识别何时需要调用工具
2. 正确生成工具调用请求（函数名 + 参数）
3. 理解工具返回结果并生成最终回答

与普通 GRPO 的关键区别:
- 多轮 Rollout: 模型可进行多轮交互（生成→调用工具→观察结果→继续生成）
- 工具调用奖励: 不仅评估最终回答，还评估工具调用的正确性
- GT（Ground Truth）验证: 检查最终回答是否包含预期答案

训练流程:
1. Rollout: 对每个 prompt 进行多轮交互式生成（最多 max_turns 轮）
2. 奖励计算: 综合工具调用正确性、GT匹配、格式规范、奖励模型评分
3. 优势估计: 组内相对排名归一化（同 GRPO）
4. 策略更新: 使用 ClSPO/GRPO 裁剪目标 + KL 惩罚

关键组件:
- Actor: 策略模型（待优化的语言模型）
- Ref Model: 参考模型（冻结，用于 KL 惩罚）
- Reward Model: 奖励模型（对无工具调用的回复打分）
- Rollout Engine: 推理引擎
- Tool Executor: 工具执行器（Mock 实现）
"""
import os
import sys
...
from trainer.trainer_utils import Logger, is_main_process, lm_checkpoint, CheckpointManager, init_distributed_mode, setup_seed, SkipBatchSampler, init_model, LMForRewardModel
...
def rollout_single(rollout_engine, tokenizer, messages, tools, max_turns=3, max_new_tokens=256, thinking_ratio=0.5, device="cuda"):
    ...
    流程:
    1. 构建当前上下文（prompt + 历史对话）
    2. 模型生成回复
    3. 解析工具调用请求
    4. 如果有工具调用: 执行工具，将结果加入上下文，继续生成
    5. 如果没有工具调用: 结束 Rollout
    ...
    Returns:
        ...
        response_mask: 回复 mask（1=模型生成，0=工具结果模板）
        response_old_logps: 旧策略的逐 token 对数概率
    ...
    prompt_ids = None
    response_ids = []
    response_mask = []        # 1=模型生成（需要计算 loss），0=模板填充（不需要 loss）
    response_old_logps = []
    final_context = ""
    unfinished = False
    open_thinking = random.random() < thinking_ratio  # 按概率开启思考
    ...
    for turn in range(max_turns):
        ...
        if prompt_ids is None:
            prompt_ids = context_ids  # 首轮记录 prompt IDs
        ...
        response_mask.extend([1] * len(new_ids))   # 模型生成的部分 mask=1
        ...
        if not calls:
            break  # 没有工具调用，结束 Rollout

        unfinished = turn == max_turns - 1  # 达到最大轮数
        ...
        for call in calls:
            ...
            result_str = (json.dumps(result, ensure_ascii=False) if result else '{"error": "tool not found"}')[:2048]  # 截断防止撑爆 tokenizer
            messages.append({"role": "tool", "content": result_str})

        # 计算工具结果模板部分的 token IDs（mask=0，不计算 loss）
        ...
        obs_delta = observe_ids[current_len:]  # 工具结果模板新增的 token
        response_ids.extend(obs_delta)
        response_mask.extend([0] * len(obs_delta))    # 模板部分 mask=0
        response_old_logps.extend([0.0] * len(obs_delta))  # 模板部分 logp=0
...
def rollout_batch(rollout_engine, tokenizer, messages_batch, tools_batch, num_gen, max_turns=3, max_new_tokens=256, thinking_ratio=0.5, device="cuda"):
    """批量多轮 Rollout: 对每个 prompt 生成 num_gen 个多轮交互

    Args:
        messages_batch: prompt 消息列表的批次
        tools_batch: 工具列表的批次
        num_gen: 每个 prompt 生成的样本数
        其他参数同 rollout_single
    Returns:
        所有样本的 Rollout 结果（展平为 B*num_gen 长度的列表）
    """
    all_completions = []
    all_contexts = []
    all_prompt_ids = []
...
def calculate_rewards(prompts, completions, gt_batch, tools_batch, num_gen, reward_model=None, device="cuda", turn_outputs_batch=None, unfinished_batch=None):
    ...
    A. 无工具调用（直接回复）:
    - 长度奖励: 5~800 字符 +0.5，否则 -0.5
    - 思考格式: 思考长度 20~300 +1.0，闭合正确 +0.25
    - 奖励模型评分: RM 对回复质量打分
    - 重复惩罚
    - 总分 Clip 到 [-3.0, 3.0]
    ...
    Args:
        ...
        gt_batch: GT 答案列表
        tools_batch: 工具列表批次
        num_gen: 每个 prompt 的生成数
        reward_model: 奖励模型（可选）
        device: 运行设备
        turn_outputs_batch: 每轮回复列表
        unfinished_batch: 未完成标记列表
    Returns:
        rewards: 奖励张量 [B*num_gen]
    """
    ...
    for idx, response in enumerate(completions):
        ...
        # 提取每轮的实际回答（去掉思考内容）
        turn_answers = [turn.split('゜', 1)[-1].strip() if '゜' in turn else turn.strip() for turn in turn_outputs]
        ...
        # 标签扣分: 工具调用标签不匹配（开始/结束标签数量不一致）
        reward -= 0.5 * sum(abs(turn.count('ゅ') - turn.count('゜')) for turn in turn_answers)
        ...
        if not tool_calls:
            ...
            if '゜' in response:
                ...
                reward += 0.25 if response.count('゜') == 1 else -0.25  # 思考闭合分
                answer = answer.strip()
...
def rl_train_epoch(epoch, loader, iters, rollout_engine, ref_model, reward_model=None, start_step=0, wandb=None, use_sglang=False):
    """Agent RL 训练一个 epoch 的主循环

    每个训练步骤:
    1. 对 batch 中的每个 prompt 进行多轮交互式 Rollout
    2. 将变长序列打包为等长张量
    3. 用当前模型和参考模型计算对数概率
    4. 计算奖励（工具调用正确性 + GT 验证 + 格式规范）
    5. 组内优势归一化
    6. 计算策略损失（ClSPO/GRPO + KL 惩罚）
    7. 反向传播 + 优化器更新
    """
    last_step = start_step
    for step, batch in enumerate(loader, start=start_step + 1):
        messages_batch = batch['messages']   # 对话消息批次
        tools_batch = batch['tools']         # 工具列表批次
        gt_batch = batch['gt']               # GT 答案批次
        last_step = step
        ...
        # ========== 第4步: 构建 completion mask（到 EOS 为止的有效 token） ==========
        ...
        if (step % args.save_interval == 0 or step == iters) and is_main_process():
            ...
            ckpt_mgr.save(
                ...
                metric_value=loss.item() if 'loss' in dir() else None,
                extra_metrics={'reward': rewards.mean().item()}
            )
            model.train()
...
if __name__ == "__main__":
    ...
    ckp_data = ckpt_mgr.load(resume_mode=args.resume_mode) if args.from_resume == 1 else None
    ...
    # 自定义 collate_fn: 将 batch 中的 messages/tools/gt 分别收集
