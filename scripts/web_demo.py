"""MiniMind Web Demo — 基于 Streamlit 的交互式聊天界面

提供美观的 Web 聊天界面，支持:
- 多语言（中文/英文）
- 流式生成输出
- 思考模式（thinking）: 折叠显示思考过程
- 工具调用（Tool Calling）: 模型可调用预定义工具
- 多轮对话历史
- 参数调节（温度、最大长度、历史轮次等）
- 自动扫描本地模型目录
...
def process_assistant_content(content, is_streaming=False):
    """处理助手回复内容，格式化思考区域和工具调用显示

    处理逻辑:
    1. 将工具调用标记格式化为美观的 HTML 卡片
    2. 将思考内容折叠显示（已思考/思考中）
    3. 处理各种思考标签的组合情况

    Args:
        content: 原始回复文本
        is_streaming: 是否为流式生成中
    Returns:
        格式化后的 HTML 文本
    """
    # 处理 tool_call 标签，格式化显示
    if 'ゅ' in content:
        ...
        content = re.sub(r'ゅ(.*?)゜', format_tool_call, content, flags=re.DOTALL)
    
    # 流式生成且开启思考时，一开始就放到折叠里
    if is_streaming and st.session_state.get('enable_thinking', False) and '゜' not in content and 'ゅ' not in content:
        m = re.search(r'(\n\n(?:我是|您好|你好)[^\n]*)', content)
    ...
    if 'ゅ' in content and '゜' in content:
        def format_think(match):
            ...
            return ''
        content = re.sub(r'(ゅ)(.*?)(゜)', format_think, content, flags=re.DOTALL)

    if 'ゅ' in content and '゜' not in content:
        def format_think_in_progress(match):
            tc = match.group(1)
            return f'<details open style="border-left: 2px solid #666; padding-left: 12px; margin: 8px 0;"><summary style="cursor: pointer; color: #888;">思考中...</summary><div style="color: #aaa; font-size: 0.95em; margin-top: 8px; max-height: 100px; overflow-y: auto; display: flex; flex-direction: column-reverse;"><div style="margin-bottom: auto;">{tc.strip().replace(chr(10), "<br>")}</div></div></details>'
        content = re.sub(r'ゅ(.*?)$', format_think_in_progress, content, flags=re.DOTALL)

    if 'ゅ' not in content and '゜' in content:
        def format_think_no_start(match):
            think_content = match.group(1)
            if think_content.replace('\n', '').strip():
                return f'<details open style="border-left: 2px solid #666; padding-left: 12px; margin: 8px 0;"><summary style="cursor: pointer; color: #888;">已思考</summary><div style="color: #aaa; font-size: 0.95em; margin-top: 8px; max-height: 100px; overflow-y: auto;">{think_content.strip()}</div></details>'
            return ''
        content = re.sub(r'(.*?)゜', format_think_no_start, content, flags=re.DOTALL)
...
def load_model_tokenizer(model_path):
    """加载模型和 tokenizer（带缓存，避免重复加载）

    Args:
        model_path: 模型路径（Transformers 格式）
    Returns:
...
def main():
    ...
    if prompt:
        ...
        full_answer = answer
        for _ in range(16):
            tool_calls = re.findall(r'ゅ(.*?)゜', answer, re.DOTALL)
            if not tool_calls:
                break
